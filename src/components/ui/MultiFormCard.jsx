...
export default function MultiFormCard({children,count,totalCount,onAdd,onRemove,isLast,icon,title,addButtonText="Add Another "}) {
    return (
        <>        
        <div className="bg-slate-50/50 border border-gray-200 rounded-xl p-4 mb-4"> 
            <div className="flex items-center justify-between">
                <div className="flex items-center ">
                    {icon && <p className="text-brand-600 font-bold">{icon}</p>}
                    <p className="text-xs font-medium text-brand-600 px-2 py-2 ">{title} #{count}</p>
                </div>
